...
class MilvusVectorSave:
    ...
    def create_connection(self):
        """创建一个Connection： milvus + langchain。先尝试远程连接，失败则回退到本地Milvus Lite"""
        for uri, label in [
            (MILVUS_URI, "远程服务器"),
            ("milvus_lite.db", "本地Milvus Lite"),
        ]:
            try:
                # 临时清理 pymilvus 可能缓存的全局配置
                ...
                log.info(f"Milvus连接成功 ({label}): {uri}")
                return
            except Exception as e:
                log.warning(f"Milvus连接失败 ({label}): {uri}, 错误: {e}")
        raise ConnectionError("所有Milvus连接方式均失败")
...
if __name__ == '__main__':
    # 解析文件内容
    file_path = r'datas/md/tech_report_example.md'
